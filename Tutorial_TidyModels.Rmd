...
With the recipe defined, let's aggregate the information in a workflow:
...
We we use the `tune_grid()` function, which calculates a set of metrics using cross-validation to evaluate performance on a pre-determined set (grid) of hyperparameters of a mode or recipe:
...
For the `ranger` engine, the [default](https://parsnip.tidymodels.org/reference/details_rand_forest_ranger.html) it `floor(sqrt(ncol(x)))`.
...
Here, we used `set_engine` and added the argument `importance = "impurity"`, so we can have variable importance scores to help us have some insight into which predictors drive model performance.
