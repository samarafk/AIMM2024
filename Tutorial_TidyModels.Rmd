...
Note that, above, we did not directly specify the degree of the polynomial. Let's choose the best hyperparameter using the `tune()` function.

With the recipe defined, let's aggregate the information:
...
The `tune_grid()` function calculates a set of metrics using cross-validation to evaluate performance on a pre-determined set of hyperparameters of a model or recipe:
...
What if we fit another model, also with some other hyperparameter? How to compare?
...
The *workflow*:
```{r}
tree_wf <- workflow(rec_simple, tree_spec) 
```
...
There is a tuning parameter that is commonly chosen for random forest models.

`mtry` is the number of predictors that will be randomly sampled at each split when creating the tree models. For the `ranger` engine, the [default](https://parsnip.tidymodels.org/reference/details_rand_forest_ranger.html) it `floor(sqrt(ncol(x)))`.
...
## Extra (more advanced): all at once
...
```{r}
wf_tune_res <-  wf_tune_set %>% workflow_map(resamples = malaria_folds, 
                             grid = 20,
...
```{r}
# Step 2: Extract the best workflow
...
And the best result for polynomial logistic regression, according to cross validation:
...
Let's assume, for instance, that between the two options (polynomial logistic regression and tree), we will stick with polynomial logistic regression. Let's select the model with the best performing hyperparameter:
...
`last_fit()` fits the final model with the training data and evaluates the performance on the test data.
