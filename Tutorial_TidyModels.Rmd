...
Este material faz parte do tutorial apresentado no AIMM, realizado no IMECC, UNICAMP.

O código fonte deste material e o conjunto de dados estão disponíveis em <https://github.com/samarafk/AIMM2024>.


# Conjunto de dados

O conjunto de dados está disponível no site <https://www.r-causal.org/chapters/02-whole-game> e <https://evalsp21.classes.andrewheiss.com/example/matching-ipw/#program-background>

Vamos carregar o pacote `tidyverse` e ler o conjunto de dados `net_data` que usaremos neste tutorial:
...
net_data <- read_csv("https://evalsp21.classes.andrewheiss.com/data/mosquito_nets.csv")

abalone <- net_data %>% select(net, malaria_risk, income, health, household, eligible, temperature, resistance)
```
...
Temos $n=`r nrow(abalone)`$ observações. A variável resposta é `malaria_risk`.
...
library(tidymodels)
set.seed(1234)
ring_split <- initial_split(abalone, prop = 0.8, strata = malaria_risk)
```
...
```{r}
ring_rec <- recipe(malaria_risk ~ ., data = ring_train)
```
...
kable_recipe <- function(rec) {
  rec %>%
    ...
    juice() %>%
    head(10) %>%
    select(malaria_risk, everything()) %>%
...
augment(reg_fit, new_data = ring_train) %>%
  abalone_metrics(malaria_risk, .pred)
```
...
ring_folds <- ring_train %>%
                vfold_cv(v = 5, strata = malaria_risk)
ring_folds
```
...
#| fig-align: center
reg_preds %>% 
  ggplot(aes(malaria_risk, .pred, color = id)) + 
...
```{r}
tree_wf <- workflow(malaria_risk ~ ., tree_spec)
tree_wf
```
...
```{r}
wf_set <- workflow_set(preproc = list(rec1 = malaria_risk ~ ., rec2 = ring_rec),
...
```{r}
workflow_set(preproc = list(rec1 = malaria_risk ~ ., rec2 = ring_rec),
             models = list(tree = tree_spec, reg = reg_spec),
             cross = TRUE) %>%
  workflow_map("fit_resamples", resamples = ring_folds) %>%
  rank_results()
...
#| code-line-numbers: "4"
ring_rec <-
  recipe(malaria_risk ~ ., data = ring_train) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_poly(temperature, degree = tune())
```
